    ...
    <div className="flex h-screen bg-[#050505] text-white font-sans overflow-hidden">
      <aside className="w-64 flex flex-col border-r border-white/5 bg-[#050505] hidden lg:flex shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-4 space-y-6 scrollbar-hide">
          ...
          <section>
            <h3 className="px-4 mb-2 text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">
              Cá nhân
            </h3>
            ...
            <div className="relative h-72 rounded-[32px] overflow-hidden group border border-white/5">
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-[#050505] via-[#050505]/60 to-transparent p-12 flex flex-col justify-center">
                <div className="flex items-center gap-2 mb-4">
                  <span className="bg-red-600 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest">
                    Đề xuất
                  </span>
                ...
                <h1 className="text-4xl md:text-5xl font-black mb-4 leading-none tracking-tight">
                  VStream <br />
                  <span className="text-red-600">Next Gen</span> Player
                </h1>
                <p className="text-gray-400 text-sm max-w-md mb-8 leading-relaxed">
                  Công nghệ truyền tải video thích ứng (HLS) kết hợp cùng hệ
                  thống xử lý phân tán giúp bạn xem video mượt mà ở mọi điều
                  kiện mạng.
                </p>
                ...
                <Link to={`/video/${v.id}`}>
                  <div key={v.id} className="group cursor-pointer">
                    <div className="relative aspect-video rounded-3xl overflow-hidden mb-5 bg-[#111] ring-1 ring-white/5 transition-all duration-500 group-hover:ring-red-600/50 group-hover:shadow-2xl group-hover:shadow-red-900/20">
                      <img
                        src={`${import.meta.env.VITE_IMAGES_URL}/${v.thumbnailUrl}`}
                        alt={v.title}
                    ...
                    <div className="flex gap-4 px-1">
                      <div className="flex-shrink-0 mt-1">
                        <div className="w-11 h-11 rounded-2xl overflow-hidden ring-2 ring-white/5 group-hover:ring-red-600/40 transition-all duration-300 shadow-lg">
                          <img
                            src={`${import.meta.env.VITE_IMAGES_URL}/${v.user.avatar}`}
...
const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  >
    <div
      className={`transition-all duration-300 ${active ? "scale-110 text-red-600" : "group-hover:scale-110 group-hover:text-red-500"}`}
    >
      {icon}
    </div>
    <span
      className={`text-[12px] font-black tracking-tight ${active ? "opacity-100" : "opacity-80 group-hover:opacity-100"}`}
    >
      {label}
    </span>
  </div>
